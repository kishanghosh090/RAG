...
# CORRECTED: Pass user_id directly into the search method
search_results = mem_client.search(query=user_query, user_id="kishan")
memory_about_user = search_results

# Handle empty or dict results safely 
if isinstance(memory_about_user, dict):
    results_list = memory_about_user.get("results", [])
else:
    results_list = memory_about_user  # Some versions return a direct list

memories = [
    f"ID {mem.get('id')}\nMemory: {mem.get('memory')}" for mem in results_list
]
